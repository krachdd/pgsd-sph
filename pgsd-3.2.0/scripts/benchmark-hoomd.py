...
"""Benchmark sGSD HOOMD file read/write."""
...
def run_benchmarks(N, size):
    ...
    if size < 64 * 1024**3:
        with pgsd.hoomd.open(name='test.gsd', mode='w') as hf:
    ...
    start = time.time()
    with pgsd.hoomd.open(name='test.gsd', mode='r') as hf:
        end = time.time()

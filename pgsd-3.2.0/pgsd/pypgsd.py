...
logger = logging.getLogger('pgsd.pypgsd')

pgsd_header = namedtuple(
    'pgsd_header',
    'magic index_location index_allocated_entries '
    'namelist_location namelist_allocated_entries '
    'schema_version pgsd_version application '
    'schema reserved',
)
pgsd_header_struct = struct.Struct('QQQQQII64s64s80s')

pgsd_index_entry = namedtuple('pgsd_index_entry',
                             'frame N location M id type flags')
pgsd_index_entry_struct = struct.Struct('QQqIHBB')
...
class PGSDFile(object):
    ...
    PGSDFile implements an object oriented class interface to the PGSD file
    ...
    :py:mod:`pgsd.fl`. Otherwise, the two implementations can be used
    interchangeably.

    Examples:
        Open a file in **read-only** mode::

            f = PGSDFile(open('file.gsd', mode='r'))
        ...
        Access file **metadata**::
            ...
            f = PGSDFile(open('file.gsd', mode='r'))
            print(f.name, f.mode, f.pgsd_version)
            print(f.application, f.schema, f.schema_version)
            print(f.nframes)

        Use as a **context manager**::

            with PGSDFile(open('file.gsd', mode='r')) as f:
                data = f.read_chunk(frame=0, name='chunk')
    """
    ...
    def __init__(self, file):
        ...
        self.__file.seek(0)
        try:
            header_raw = self.__file.read(pgsd_header_struct.size)
        except UnicodeDecodeError:
            print("\nDid you open the file in binary mode (rb)?\n",
                  file=sys.stderr)
            raise

        if len(header_raw) != pgsd_header_struct.size:
            raise IOError

        self.__header = pgsd_header._make(pgsd_header_struct.unpack(header_raw))

        # validate the header
        if self.__header.magic != 0x65DF65DF65DF65DF:
            raise RuntimeError("Not a PGSD file: " + str(self.__file))
        if (self.__header.pgsd_version < (1 << 16)
                and self.__header.pgsd_version != (0 << 16 | 3)):
            raise RuntimeError("Unsupported PGSD file version: "
                               + str(self.__file))
        if self.__header.pgsd_version >= (3 << 16):
            raise RuntimeError("Unsupported PGSD file version: "
                               + str(self.__file))
        ...
        for i in range(self.__header.index_allocated_entries):
            index_entry_raw = self.__file.read(pgsd_index_entry_struct.size)
            if len(index_entry_raw) != pgsd_index_entry_struct.size:
                raise IOError

            idx = pgsd_index_entry._make(
                pgsd_index_entry_struct.unpack(index_entry_raw))
            ...
            if not self.__is_entry_valid(idx):
                raise RuntimeError("Corrupt PGSD file: " + str(self.__file))
    ...
    def __is_entry_valid(self, entry):
        ...
        if entry.type not in pgsd_type_mapping:
            return False
    ...
    @property
    def pgsd_version(self):
        """tuple[int, int]: PGSD file layer version number.

        The tuple is in the order (major, minor).
        """
        v = self.__header.pgsd_version

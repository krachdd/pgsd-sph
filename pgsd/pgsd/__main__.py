...
"""The PGSD command line interface.

To simplify ad hoc usage of :py:mod:`pgsd`, this module provides a command line
interface for interacting with PGSD files. The primary entry point is a single
command for starting a Python interpreter with a PGSD file pre-loaded::

    $ pgsd read trajectory.pgsd
...
.. option:: -s schema, --schema schema
    ...
    The schema of the PGSD file. Supported values for ``schema`` are "hoomd" and
    "none".

.. option:: -m mode, --mode mode

    The mode in which to open the file. Valid modes are identical to those
    accepted by :func:`pgsd.fl.open`.
"""
...
File: {fn}
{extras}
The PGSD file handle is available via the "handle" variable.
...
The pgsd and pgsd.fl packages are always loaded.
Schema-specific modules (e.g. pgsd.hoomd) are loaded if available."""


def main_read(args):
    """Main function to launch a Python interpreter with an open PGSD file."""
    # Default to a new line for well-formatted printing.
    local_ns = {
        'pgsd': sys.modules['pgsd'],
        'pgsd.hoomd': sys.modules['pgsd.hoomd'],
        'pgsd.fl': sys.modules['pgsd.fl'],
    }
    attributes = {}
...
def main():
    """Entry point to the PGSD command-line interface.

    This function handles parsing command-line arguments and launching the
    appropriate subcommand based on the first argument to ``pgsd`` on the
    command line. At present the following commands are supported:

        * read
    """
    parser = argparse.ArgumentParser(
        description="The pgsd package encodes canonical readers and writers "
        "for the pgsd file format.")
